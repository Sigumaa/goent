...
func (e TweetEdges) UsersOrErr() (*User, error) {
	if e.loadedTypes[0] {
		if e.Users == nil {
...
func (*Tweet) scanValues(columns []string) ([]interface{}, error) {
	...
	for i := range columns {
		...
		case tweet.FieldTitle, tweet.FieldContent:
...
// QueryUsers queries the "users" edge of the Tweet entity.
func (t *Tweet) QueryUsers() *UserQuery {
	return (&TweetClient{config: t.config}).QueryUsers(t)
}
